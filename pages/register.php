<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $fioValue = trim($_POST['fio'] ?? '');
        $phoneValue = trim($_POST['phone'] ?? '');
        $password = $_POST['password'] ?? '';

        $controller = new RegisterController();
        $error = $controller->register($fioValue, $phoneValue, $password);
    }
?>
<!DOCTYPE html>
<html lang="ru">
...
<body>
    <div class="shell">
        <section class="hero" aria-label="Описание регистрации">
            ...
            <p class="lead">
                Регистрация займёт меньше минуты. После этого вы сможете оформлять заказы.
            </p>
        ...
        <section class="card" aria-label="Форма регистрации">
            <h2>Регистрация</h2>
    ...
    <script>
        (function () {
            ...
            fio.addEventListener('input', function () {
                const start = fio.selectionStart;
                const before = fio.value;
                const cleaned = sanitizeFio(before);
                if (cleaned !== before) {
                    fio.value = cleaned;
                    const diff = before.length - cleaned.length;
                    const pos = Math.max(0, start - diff);
                    fio.setSelectionRange(pos, pos);
                }
            });

            fio.addEventListener('paste', function (e) {
                e.preventDefault();
                const text = (e.clipboardData || window.clipboardData).getData('text');
                const cleaned = sanitizeFio(text);
                const start = fio.selectionStart;
                const end = fio.selectionEnd;
                fio.value = fio.value.slice(0, start) + cleaned + fio.value.slice(end);
